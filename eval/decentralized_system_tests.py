...
class TestDecentralizedSystem(unittest.TestCase):
    def setUp(self):
        ...
        # Añadido por nosotros para que no se mezclen los tests
        files = ['32770_data.json', '32771_data.json', '32772_data.json'] 
        for file in files: 
    ...
    def test_state_recovery_after_critical_failure(self):
        ...
        try:
            ...
            time.sleep(5) # Añadido por nosotros para que de tiempo a hacer las backups
            self.stop_grpc_server()
            time.sleep(5)
            self.server_process = self.start_grpc_server()
            time.sleep(10)
